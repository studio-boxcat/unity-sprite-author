...
- **Byte-exact `CanvasSpriteAuthor.Publish()` reproduction.** Fixtures at `tests/golden/fab/silloutte/`. The crate infrastructure landed (Phases 1–7 done — parser, polygon, ID, MX/MY/MXY dual-strategy, TX/TY/TX_MC3, all 17 slice grids, fabricated emit path, pipeline wiring). What's outstanding is *authoring*, not coding:
    - Hand-derive each Silloutte part's `tx`/`ty`/`partPivot` from the prefab's `RectTransform.{anchoredPosition, sizeDelta, pivot}` and `CanvasSpriteAuthor._scaleFactor` (= 0.01). For UIIcon (= native-scale `Id`/`Mx`/`My`/`Mxy`), `sx`/`sy` absorb the UIIcon `_scaleFactor`. For UISolid (= `Part::Polygon` with 4 corner verts of `sizeDelta * scaleFactor`), pre-compute the four `vertices` entries.
    - Cross-check that `combine::calc_rect_and_pivot` derives the same `m_Rect` (282.5×770) and `m_Pivot` (0.5, 0.40551946) as `Silloutte1.asset`.
    - Iterate until `pipeline::generate` emits bytes equal to `Silloutte1.asset` byte-for-byte (the existing `pipeline_emits_combined_sprite_and_excludes_parts` test demonstrates the wiring).
