...
- **Byte-exact `CanvasSpriteAuthor.Publish()` reproduction.** Fixtures at `tests/golden/fab/silloutte/`. Manifest hand-authored at `PremiumCat_Vampire_Popup.tps.fab.json` (Silloutte1 covered; Silloutte2/3 pending). Test scaffold at `tests/golden_fab_silloutte.rs` (`#[ignore]`d; run with `--include-ignored`). Mesh structure matches byte-exactly across all parts EXCEPT for sub-ULP f32 rounding differences. Status:
    - **Mesh AABB, m_Rect, m_Pivot, m_Offset, indexCount, vertexCount, dataSize, index buffer (all 300 indices)**: byte-exact match.
    - **typelessdata vertex positions**: ~1-ULP f32 differences in a few verts (e.g. `0.587499976` vs `0.587500036` — same logical value, different f32 rounding through `(47/100)*1.25` vs Unity's path). All other verts byte-exact.
    - **File size**: mine 16405 bytes, golden 16409 (4-byte difference, likely from a slightly-longer float string somewhere — possibly uvTransform.w which Silloutte uses non-trivially).
    - **Root cause of the residual 1-ULP diffs** (probed via llm-bridge):
      Unity stores `Sprite.vertices[i]` as `(atlas_pixel - pivot_pixel) * (1/ppu)` (multiply-by-precomputed-reciprocal — fixed in `local_src_verts`). Then `CanvasSpriteAuthor.GenerateMesh` does the f32 round-trip `((srcPos × UIIcon._scaleFactor=100) + localPosition) × CanvasSpriteAuthor._scaleFactor=0.01`. The 100/0.01 round-trip introduces 1-ULP rounding noise (e.g. for B vert (58, 0): `0.725 × 100 = 72.5` exact → `× 0.01 = 0x3f399999` round-down vs my collapsed `0.725 = 0x3f39999a` round-up). Mathematically identical, 1-ULP differing at the f32 bit level.
    - **Path forward**: extend the fab schema with an explicit two-step transform — a per-part `uiScale` (= UIIcon `_scaleFactor`) and a per-combined `canvasScale` (= CanvasSpriteAuthor `_scaleFactor`). The pipeline then composes `((srcPos × uiScale) + offset) × canvasScale` in that exact f32 order. Schema and emit code both need plumbing. Estimated ~2-3 hours of work to reach actual byte-exact.
