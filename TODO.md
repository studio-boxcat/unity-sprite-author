...
- **Byte-exact `CanvasSpriteAuthor.Publish()` reproduction.** Fixtures at `tests/golden/fab/silloutte/`. Manifest hand-authored at `PremiumCat_Vampire_Popup.tps.fab.json` (Silloutte1 covered; Silloutte2/3 pending). Test scaffold at `tests/golden_fab_silloutte.rs` (`#[ignore]`d; run with `--include-ignored`). Mesh structure matches byte-exactly across all parts EXCEPT for sub-ULP f32 rounding differences. Status:
    ...
    - To close the remaining gap: trace Unity's exact f32 op sequence in `Sprite.vertices → UIIconMeshGen.Identity/MX → CombineMeshes(transform=baseMatrix * localToWorldMatrix)` and reproduce the operand order verbatim. Likely intermediate via `sprite.bounds.size` (= rect/ppu with f32 rounding) rather than the on-the-fly computation. Probably need a Unity-side probe harness that logs every intermediate vert.
