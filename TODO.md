...
- **Per-part RectTransform pivot in the schema.** The `CanvasSpriteAuthor` golden fixtures (see `tests/golden/fab/silloutte/README.md`) use mixed per-part pivots (`(0.5, 0.5)`, `(0, 0.5)`, `(0.5, 0)`) which shift each part's slice-method output relative to its anchored position. The current schema hardcodes the slice target-rect pivot to `(0.5, 0.5)`. Add an optional `partPivot: [x, y]` on the atlas-sprite part variant; default `(0.5, 0.5)` matches the SpriteRenderer-tree use case.
- **Fabricated-sprite emit path.** Current `emit::SpriteAsset` is wired for tpsheet-imported sprites (`atlasRectOffset: -1, -1`; `m_Rect.{x,y}` = atlas pixels). `CanvasSpriteAuthor.Publish()` emits sprites with `atlasRectOffset: 0, 0`, `m_Rect.{x,y} = 0`, `m_Rect.{w,h}` = mesh AABB. Either parameterize `SpriteAsset` with a `Source { Tpsheet, Fabricated }` discriminator or split into two emit paths. Required before any byte-exact fab golden can run.
