...
- ~~**Byte-exact `CanvasSpriteAuthor.Publish()` reproduction.**~~ **SOLVED** for Silloutte1 (`tests/golden_fab_silloutte.rs` passes byte-exact under `--include-ignored`). Fix involved four changes:
    ...
    - **Remaining**: Silloutte2 byte-exact now (also covered by the test); Silloutte3 has the manifest authored but the test still fails with a uniform 1–2 ULP shift on every y-coordinate (60/184 position-stream f32s differ; UVs all match). Silloutte3 is the only one of the three whose **root** GameObject has a non-(0, 0) `m_AnchoredPosition` (`(141.8, 370.875)`); Silloutte1 + Silloutte2 roots sit at the origin and pass byte-exactly. Hypothesis: Unity's per-child transform multiplies through the parent chain so the matrix's translation row carries `root.anchored × canvas_scale = 3.70875` of precision-eating noise, and the round-trip when Sprite emit re-localizes into the sprite's own frame shifts every vert by ~1 ULP at scale 1.0 (= 2⁻²³). Cannot be reproduced from a pure-Rust f32 chain; needs a Unity-side probe to read back the exact `Matrix4x4` Unity bakes for a Silloutte3-shape parent chain. Tracked alongside the other Unity probes in [[unity-probes.md]].
